...
bool CAimbotHitscan::ShouldFireByX(CTFPlayer* pLocal, CTFWeaponBase* pWeapon, CUserCmd* pCmd)
{
	...
	// Create and use proper trace filter
	...
	if ((Vars::Aimbot::General::Ignore.Value & Vars::Aimbot::General::IgnoreEnum::Friends && H::Entities.IsFriend(pPlayer->entindex()))
		...
		|| (Vars::Aimbot::General::Ignore.Value & Vars::Aimbot::General::IgnoreEnum::Cloaked && pPlayer->InCond(TF_COND_STEALTHED))
		...
		|| (Vars::Aimbot::General::Ignore.Value & Vars::Aimbot::General::IgnoreEnum::Vaccinator && pPlayer->InCond(TF_COND_MEDIGUN_UBER_BULLET_RESIST)))
	{
		return false;
	}
...
void CAimbotHitscan::Run(CTFPlayer* pLocal, CTFWeaponBase* pWeapon, CUserCmd* pCmd)
{
	...
	for (auto& tTarget : vTargets)
	{
		...
		case TargetEnum::Player:
		{
			...
			{
				// Use simple bone selection without passing function pointer
				int pBone = F::AimbotGlobal.IsHitboxValid(H::Entities.GetModel(pPlayer->entindex()), iCanHit);
				if (pBone)
			...
			G::AimTarget.m_iTickCount = nCurPlay->m_nTickBase();
			return;
		}
